...
async def run_generate(md_path: Path, model: str, limit: int, retries: int) -> int:
    ...
    try:
        ...
        print(f'[HINT] Expected module: {SCRIPT_DIR / "gemini_api_generator.py"}')
        return 1
...
def main() -> int:
    ...
    if args.batch_date:
        date_dir = WECHAT_CONTENT_ROOT / args.batch_date
        if not date_dir.exists():
            date_dir = LEGACY_WECHAT_CONTENT_ROOT / args.batch_date
